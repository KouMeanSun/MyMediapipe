...
EXPORT_API int Mediapipe_Hand_Tracking_Detect_Frame_Direct(int image_width, int image_height, void* image_data,GestureRecognitionResult& gesture_result,int isHorizonFlip,int upDownthresholdValue,int leftRightthresholdValue, PoseInfo* firstInfos,PoseInfo* secondInfos, bool& isGestureOK)
{
	return m_HandTrackingDetect.DetectFrame_Direct(image_width, image_height, image_data, gesture_result,isHorizonFlip,upDownthresholdValue, leftRightthresholdValue,firstInfos,secondInfos, isGestureOK);
}
...
EXPORT_API int Mediapipe_Hand_Tracking_Box_Register(int hotBoxLeftTopX,int hotBoxLeftTopY,int hotBoxRightBottomX,int hotBoxRightBottomY ,int okGester){
	return m_HandTrackingDetect.BoxRegister(hotBoxLeftTopX,hotBoxLeftTopY,hotBoxRightBottomX,hotBoxRightBottomY,okGester);
}